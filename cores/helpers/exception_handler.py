...
from cores.schemas.sche_base import ResponseSchemaBase
...
class CustomResponse(Exception):
    http_code: int
    code: str
    message: str
...
def http_exception_handler(request: Request, exc: CustomResponse):
    return JSONResponse(
...
def fastapi_error_handler(request, exc):
    return JSONResponse(
        status_code=500,
